...
function ModuloItem({ module, onDelete, onEdit}) {
  const {nombre, orden, temas, evaluaciones, descripcion} = module;
  ...
  return (
    <article className="grid items-center px-0 py-10 bg-white rounded-lg grid-cols-[2fr_1fr_1fr_1fr_2fr_0.5fr] max-md:p-4 max-md:grid-cols-[1.5fr_1fr_1fr_1fr_1.5fr_0.5fr] max-sm:gap-2.5 max-sm:p-2.5 max-sm:grid-cols-[1fr] relative">
      ...
      <div className="text-sm text-center text-black">{orden}</div>
      ...
      <button
          aria-label="Delete"
          onClick={() => onEdit(module)} // Llamamos a onDelete pasando la unidad
        ...
        <button
          aria-label="Delete"
          onClick={() => onDelete(module)} // Llamamos a onDelete pasando la unidad
          className="ti ti-trash text-2xl cursor-pointer"
        >
          <FaTrashAlt />
        </button>
